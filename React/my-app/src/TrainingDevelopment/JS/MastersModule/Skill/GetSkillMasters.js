...
const GetSkillMasters = () => {
  ...
  const [data, setData] = useState([]);
    ...
    const editComp = (id) =>{
         navigate(`/updateSkillMasters/${id}`)
    }

    const loadData = async() =>{
        let response = await axios.get('http://localhost:8080/getSkillNameDetails')
  ...
  return (
    <div>
      <Sidebar show='true'/>
      <div className='absolutes'>Master's Skill</div>
      <div style={{marginTop:'-5px'}} className="shadowBgWhite ms-1">
        ...
        <div style={{marginTop:'-20px',marginLeft:'478px'}}><button className="addEmpButtuon ps-3" onClick={()=>navigate('/addSkillMasters')}>
          <MdOutlineAddCircleOutline/> Add Skill</button></div>
        </div> <br/> 
        ...
        <tbody style={{backgroundColor:'white'}}>
          {data.filter((val)=>{
           if(search == ""){
            ...
            return val
           }else if(val.skill.toLowerCase().includes(search.toLowerCase())){
              return val
            ...
            <tr key={id}>
              ...
              <td style={{paddingTop:'15px',paddingBottom:'15px'}}>{e.skill}</td>
...
export default GetSkillMasters;
