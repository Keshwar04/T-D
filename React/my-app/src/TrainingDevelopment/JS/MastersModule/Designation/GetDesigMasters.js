...
const GetDesigMasters = () => {
  ...
  const [search,setSearch] = useState([]);
    ...
    const editComp = (id) =>{
         navigate(`/updateDesgMasters/${id}`)
    }

    const loadData = async() =>{
        let response = await axios.get('http://localhost:8080/getDesgDetails')
  ...
  return (
    <div>
      <Sidebar show='true'/>
      <div className='absolutes'>Master's Designation</div>
      <div style={{marginTop:'-5px'}} className="shadowBgWhite ms-1">
        ...
        <div style={{marginTop:'-20px',marginLeft:'478px'}}><button className="addEmpButtuon ps-2" onClick={()=>navigate('/addDesgMasters')}>
          <MdOutlineAddCircleOutline/> Add Designation</button></div>
        </div> <br/> 
          ...
          {data.filter((val)=>{
           if(search == ""){
            ...
            <tr key={id}>
              ...
              <td style={{paddingTop:'15px',paddingBottom:'15px'}}>{e.designation}</td>
...
export default GetDesigMasters;
